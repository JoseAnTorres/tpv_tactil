...
{
    if (add_referencia_lock == true) {
        return;
    }

    add_referencia_lock = true;
    $.ajax({
        type: 'POST',
        url: tpv_url,
        dataType: 'html',
...
function send_ticket()
{
    if (document.f_tpv.codbar.value == '') {
        ...
        $.ajax({
            ...
            url: tpv_url,
            dataType: 'html',
            data: 'codbar2=' + document.f_tpv.codbar.value + '&numlineas=' + numlineas + '&codcliente=' + document.f_tpv.cliente.value,
            success: function (datos) {
                if (datos.indexOf('<!--no_encontrado-->') != -1) {
                    bootbox.alert('¡Artículo no encontrado!');
        ...
        $.getJSON(tpv_url, $("form[name=f_buscar_articulos]").serialize(), function (json) {
            ...
            $.each(json, function (key, val) {
                ...
                if (val.sevende && (val.stockalm > 0 || val.controlstock)) {
                  ...
                  <td class=\"text-right\"><a href=\"#\" onclick=\"" + funcion + "\">" + show_pvp_iva(val.pvp, val.codimpuesto) + "</a></td>\n\
                  <td class=\"text-right\">" + stock + "</td></tr>");
                } else if (val.sevende) {
                    items.push(tr_aux + "<td>\n\
                  <a href=\"#\" onclick=\"alert('Sin stock.')\">" + val.referencia + '</a> ' + val.descripcion + "</td>\n\
                  <td class=\"text-right\"><a href=\"#\" onclick=\"alert('Sin stock.')\">" + show_pvp_iva(val.pvp, val.codimpuesto) + "</a></td>\n\
                  <td class=\"text-right\">" + stock + "</td></tr>");
                }
